<template>
  <el-drawer v-model="settingsVisible" size="300" :title="$t('settings.project')">
    ...
    <div class="settings-option">
      ...
      <el-switch v-model="(settingsStore as any).tagsView" />
    </div>

    <div class="settings-option">
      <span class="text-xs">{{ $t('settings.fixedHeader') }}</span>
      <el-switch v-model="(settingsStore as any).fixedHeader" />
    </div>

    <div class="settings-option">
      <span class="text-xs">{{ $t('settings.sidebarLogo') }}</span>
      <el-switch v-model="(settingsStore as any).sidebarLogo" />
    </div>

    <div class="settings-option">
      <span class="text-xs">{{ $t('settings.watermark') }}</span>
      <el-switch v-model="(settingsStore as any).watermarkEnabled" />
    </div>
